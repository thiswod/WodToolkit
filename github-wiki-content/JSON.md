...
// 方式一：直接发送匿名对象（自动序列化）
httpRequest.Open("https://api.example.com/users", HttpMethod.Post).Send(new
{
    name = "John",
    age = 25,
    email = "john@example.com"
...
```csharp
var httpRequest = new HttpRequestClass();
httpRequest.Open("https://api.example.com/users/1", HttpMethod.Get).Send();
var response = httpRequest.GetResponse();

// 解析响应 JSON
dynamic user = EasyJson.ParseJsonToDynamic(response.Body);
Console.WriteLine($"用户名: {user.name}");
Console.WriteLine($"年龄: {user.age}");

// 或解析为强类型
var userObj = EasyJson.ParseJsonObject<User>(response.Body);
```
